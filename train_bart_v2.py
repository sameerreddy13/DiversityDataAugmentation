...
def compute_rewards(inputs: List[str], ouputs: List[str]) -> torch.FloatTensor:
  '''
...
def main():
  ...
  for epoch in range(epochs):
    with tqdm(train_loader, unit="batch", disable=True) as tepoch:
      ...
      for batch in tepoch:
          ...
          out = rl_model.generate_episodes(batch, max_length=max_in_len+1, epsilon=epsilon)
          # Compute rewards
